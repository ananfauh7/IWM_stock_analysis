...
try:
    ...
    if 'error' in analysis:
        ...
        st.info("The AI strategy recommendations will be available once the OpenAI API key is configured.")
    else:
        # Calculate strategy confidence score
        confidence_score = get_strategy_confidence_score(analysis['market_data'])
        ...
        for signal in analysis['technical_signals']:
            signal_color = 'green' if 'Bullish' in signal else 'red' if 'Bearish' in signal else 'blue'
    ...
    st.error(f"An error occurred while generating the AI strategy: {str(e)}")
    st.info("Please try refreshing the page. If the error persists, check if all required services are properly configured.")